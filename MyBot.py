...
def is_in_short_targets(target):
    for ship_id in short_targets:
        if short_targets[ship_id]['target'] is target:
            return True
    return False
...
def set_short_target(ship, target):
    ...
    for direction in directions:
        ...
        if prospect_distance < min_distance and not is_in_short_targets(prospect_position):
...
def set_long_target(ship):
    ...
    for y in range(game_map.height):
        for x in range(game_map.width):
            ...
            if not has_arrival_conflict(ship, cell.position):
                cell_distance = game_map.calculate_distance(ship.position, cell.position)
                cell_halite = cell.halite_amount
                prospect_value = cell_halite - cell_distance*k_distance_multiplier
